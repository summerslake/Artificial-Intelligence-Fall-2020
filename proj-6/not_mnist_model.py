...
class Model(object):
    def __init__(self, learning_rate):
        ...
        #######################################################################
        self.param = {
		    #Declare 1 hidden layer
            "w1": nn.parameter.from_numpy(numpy.random.normal(0, 0.01, [n_features, HIDDEN_UNITS])),
            "w2": nn.parameter.from_numpy(numpy.random.normal(0, 0.01, [HIDDEN_UNITS, HIDDEN_UNITS])),
            "w3": nn.parameter.from_numpy(numpy.random.normal(0, 0.01, [HIDDEN_UNITS, n_classes])),

            "b1": nn.parameter.zeros([HIDDEN_UNITS]),
            "b2": nn.parameter.zeros([HIDDEN_UNITS]),
    ...
    def forward_propagation(self, x):
        ...
        z1 = nn.matmul(x,self.param["w1"]) + self.param["b1"]
        a1 = nn.relu(z1)
        z2 = nn.matmul(a1,self.param["w2"]) + self.param["b2"]
        a2 = nn.relu(z2)
        logits = nn.matmul(a2,self.param["w3"]) + self.param["b3"]
    ...
    def get_loss(self, x, y):
        ...
        #######################################################################
        "*** YOUR CODE HERE ***"
        [loss, probs] = nn.softmax_cross_entropy_loss(self.forward_propagation(x), y)
        #######################################################################
    ...
    try:
        ...
        for epoch in range(1, MAX_EPOCHS+1):
            ...
            for step, (x, y) in enumerate(DB.train, 1):            
                ...
                nn.SGD([model.param["w1"],model.param["b1"]], LEARNING_RATE)

                #compute gradient of the loss and perform gradient update
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()

                nn.SGD([model.param["w2"],model.param["b2"]], LEARNING_RATE)
                ...
                nn.SGD([model.param["w3"],model.param["b3"]], LEARNING_RATE)
                ...
                n_correct = model.get_correct_predictions(probs, y).item()
                accuracy = n_correct
                global_step = global_step + 1
                logger.log(loss.item(), accuracy, global_step, False)
                #######################################################################
