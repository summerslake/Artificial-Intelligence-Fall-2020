# Nicholas Gustafson and Lake Summers
# CPSC 4420
# Due: Sep 8 2020
...
#############################################################################
    ...
    while len(open_set) > 0:
        ...
        node = open_set.pop()
        #Set nodeX and nodeY to the current node
        nodeX, nodeY = node
        #Check if the goal is found
        if node == goal:
            #Back track to the start
            while node != start:
                #Insert the movement for visualization
        ...
        for action in ACTIONS:
            #Get the x and y of the action
            actionX, actionY = action
            #Add the actionX and actionY to the current node to get a childX and childY
            childX = actionX + nodeX
            childY = actionY + nodeY
            #Update the child node
            child = (childX, childY)
            #If the child is within bounds and not an obstacle
            if (childX >= 0 and childX < n_rows and
                childY >= 0 and childY < n_cols and
                child not in obstacles):
                #If child isn't in either set
                if child not in open_set and child not in closed_set:
                    #Put the child into the open set
                    open_set.add(child)
                    #Assign node as child's parent
                    parent[childX][childY] = node
                    #Add action taken to actions
                    actions[childX][childY] = action
#############################################################################
    ...
    while len(open_set) > 0:
        ...
        for action in ACTIONS:
            ...
            if (childX >= 0 and childX < n_rows and
                ...
                child not in obstacles):
                #Get the cost
                childCost = costFn(child)
                #If child isn't in either set
                if child not in open_set and child not in closed_set:
                    #Put the child and it's cost into the open set
                    open_set.put(child, childCost)
                    #Assign node as the child's parent
                    parent[childX][childY] = node
                    #Add action taken to actions
                    actions[childX][childY] = action
                #If the child is in the open set and the cost is less than already noted
                elif child in open_set and childCost < open_set.get(child):
                    #Add the child and it's new cost
                    open_set.put(child, childCost)
                    #Assign node as the child's parent
                    parent[childX][childY] = node
                    #Add action taken to actions
    ...
    while len(open_set) > 0:
        ...
        for action in ACTIONS:
            ...
            if (childX >= 0 and childX < n_rows and
                ...
                #If child isn't in either set
                if child not in open_set and child not in closed_set:
                    #Put the child along with it's cost (taking into account manhattan distance)
                        #into the open set
                    open_set.put(child, childCost + heuristic(childX, childY))
                    #Assign node as the child's parent
                    parent[childX][childY] = node
                    #Add action taken to actions
                    actions[childX][childY] = action
